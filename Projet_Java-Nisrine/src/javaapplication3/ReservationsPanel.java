...
class DatabaseHelper {
    private static final String URL = "jdbc:mysql://localhost:3306/unilib";
...
public class ReservationsPanel extends RoundedPanel {
    ...
    private RoundedTextField cneField, livreField;
    private JDateChooser dateReserveeField, dateLimiteField;
    private RoundedTextField searchField;
    private RoundedButton validerBtn;
    ...
    private JPanel createSearchPanel() {
        ...
        searchField = new RoundedTextField("Entrez le CNE");
        ...
        ImageIcon searchIcon = new ImageIcon(getClass().getResource("/icons/search.png")); // mets ton chemin exact
    ...
    private JPanel createFormPanel() {
        ...
        cneField = new RoundedTextField();
        livreField = new RoundedTextField();
        ...
        addFormRow(panel, gbc, 0, "CNE", cneField, "Date réservée", wrapWithRoundedPanel(dateReserveeField));
        addFormRow(panel, gbc, 1, "Livre", livreField, "Date limite", wrapWithRoundedPanel(dateLimiteField));
    ...
    private JPanel createButtonPanel() {
        validerBtn = new RoundedButton("Valider");
    ...
    private void handleReservationAction() {
        ...
        try (Connection conn = DatabaseHelper.getConnection()) {
            ...
            if (isEditing) {
                ...
                if (docInfo.getAvailableCopies() <= 0) {
                    showError("Aucun exemplaire disponible pour ce document !");
                    conn.rollback();
                    return;
                }
    ...
    private DocumentInfo findDocument(Connection conn, String numInventaire) throws SQLException {
        String[] tables = {"livre", "memoire_master", "rapport_licence", "these_doctorat", "document"};
    
        for (String table : tables) {
            String sql = "SELECT nbr_exemplaires FROM " + table + " WHERE num_inventaire = ?";
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                ...
                // En cas d'erreur sur une table manquante ou mal formée, on continue.
                Logger.getLogger(getClass().getName()).log(Level.WARNING, "Erreur dans findDocument pour table: " + table, e);
            }
        }
    
        return new DocumentInfo(); // Aucun document trouvé
    }
    ...
    private void updateDocumentStocks(Connection conn, String numInventaire, String table, int change) throws SQLException {
        // 1. Mettre à jour la table spécifique
        try (PreparedStatement stmt = conn.prepareStatement(
                "UPDATE " + table + " SET nbr_exemplaires = nbr_exemplaires + ? WHERE num_inventaire = ?")) {
            stmt.setInt(1, change);
            stmt.setString(2, numInventaire);
            stmt.executeUpdate();
        }

        // 2. Mettre aussi à jour la table `document`
        try (PreparedStatement stmtDoc = conn.prepareStatement(
                "UPDATE document SET nbr_exemplaires = nbr_exemplaires + ? WHERE num_inventaire = ?")) {
            stmtDoc.setInt(1, change);
            stmtDoc.setString(2, numInventaire);
    ...
    private class StatusCellRenderer extends DefaultTableCellRenderer {
        ...
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected,
            ...
            RoundedLabel label = new RoundedLabel(value != null ? value.toString() : "");
    ...
    private JPanel wrapWithRoundedPanel(JComponent component) {
        RoundedPanel panel = new RoundedPanel(new Color(255, 255, 255, 230), 20); // blanc semi-transparent
        panel.setLayout(new BorderLayout());
        panel.setOpaque(false);
        panel.setBorder(BorderFactory.createEmptyBorder(2, 5, 2, 5));
        component.setOpaque(false); // Important pour éviter le fond gris du JDateChooser
